...
|-----|--------------------------------------------|--------------------------------------------------------------------------------------------|-----|-----------|-----------|--------|
| S42 | Model organizacije                         | Definisanje modela organizacije koja koristi sistem                                        | T   | P1        | M         | DONE   |
| S43 | Model korisnika                            | Definisanje korisničkog modela i veze sa firmom                                            | T   | P1        | M         | DONE   | 
| S44 | Registracija firme                         | Omogućavanje registracije nove firme u sistemu                                             | F   | P1        | M         | DONE   | 
| S45 | Admin nalog                                | Kreiranje prvog administratorskog korisnika firme                                          | F   | P1        | M         | DONE   | 
| S46 | Login                                      | Implementacija prijave korisnika u sistem                                                  | F   | P1        | M         | DONE   | 
| S47 | Logout                                     | Omogućavanje odjave korisnika                                                              | F   | P1        | S         | DONE   | 
| S48 | Auth sigurnost                             | Implementacija sigurnosti (hashiranje lozinki, tokeni)                                     | T   | P1        | M         | DONE   | 
| S49 | Multi-tenant zaštita                       | Osiguranje da korisnici vide samo podatke svoje firme                                      | T   | P1        | M         | DONE   | 
| S50 | Test auth                                  | Testiranje autentifikacije i autorizacije                                                  | T   | P2        | M         | DONE   | 
| S51 | Role model                                 | Definisanje korisničkih rola u sistemu                                                     | T   | P1        | M         | DONE   | 
| S52 | Kreiranje korisnika                        | Omogućavanje adminu da dodaje nove korisnike                                               | F   | P1        | M         | DONE   | 
| S53 | Dodjela rola                               | Dodjela rola korisnicima                                                                   | F   | P1        | M         | DONE   |
| S54 | Test workflow                              | Testiranje workflow logike i rola                                                          | T   | P2        | M         | DONE   | 
| S55 | Reset password                             | Omogućavanje resetovanja lozinke                                                           | F   | P2        | M         | DONE   | 
| S56 | Dashboard                                  | Prikaz osnovnih statistika sistema                                                         | F   | P3        | M         | DONE   | 
| S57 | User management                            | Upravljanje korisničkim podacima                                                           | F   | P2        | M         | DONE   |
| S58 | Proširenje procesa za upload i ekstrakciju | Dodatni tipovi dokumenata, ekstrakcija na osnovu tipa dokumenta i automatska klasifikacija | F   | P1        | M         | DONE   |
...
- Kada administrator dodijeli korisniku rolu (operater, menadžer, odobravatelj), tada sistem mora sačuvati rolu
...
**Opis** - Kao administrator, želim resetovati lozinku korisničkih naloga, kako bi korisnicima vratio pristup nalogu.

**Acceptance Criteria**

- Kada administrator pokrene proceduru resetovanja lozinke, tada sistem mora generisati novu privremenu lozinku kako bi
  korisnik mogao ponovo pristupiti svome nalogu.
- Sistem mora osigurati da stara lozinka više ne važi nakon uspješnog resetovanja.
- Administrator treba dobiti potvrdu da je lozinka uspješno promijenjena.
...
**Opis** - Kao operater, želim pri uploadu odabrati precizniji tip dokumenta, kako bi sistem mogao primijeniti
odgovarajući OCR/AI tok obrade umjesto osnovne MVP podjele na invoice i ostalo.
...
- Kada korisnik odabere Other/Auto classify, tada sistem mora tretirati dokument kao kandidat za automatsku
  klasifikaciju.
...
**Opis** - Kao korisnik sistema, želim da sistem obradi različite tipove dokumenata odgovarajućim OCR/AI procesorom,
kako bi ekstrakcija bila preciznija nego u osnovnoj MVP verziji obrade.
...
- Postojeći extraction flow mora ostati isti za korisnika, bez uvođenja odvojenih stranica ili potpuno različitih tokova
  po tipu dokumenta.
...
**Opis** - Kao operater, želim da sistem automatski pokuša prepoznati stvarni tip dokumenta kada dokument nije unaprijed
precizno označen, kako bih imao bolji tok obrade od osnovne klasifikacije “invoice ili ostalo”.

**Acceptance Criteria**

- Kada je dokument uploadovan kao Other/Auto classify, tada sistem mora prvo pokrenuti classifier procesor.
- Ako classifier prepozna podržan tip dokumenta sa dovoljnom confidence vrijednošću, tada sistem mora postaviti
  detektovani tip dokumenta.
- Ako classifier prepozna invoice, receipt, bank statement ili form sa dovoljnom sigurnošću, tada sistem mora nastaviti
  ekstrakciju odgovarajućim parserom.
...
- Kada classifier nije dovoljno siguran, dokument mora preći u status koji jasno označava da je potreban ručni pregled
  tipa.
...
**Opis** - Kao operater, želim ručno potvrditi tip dokumenta kada ga sistem ne može dovoljno sigurno klasifikovati, kako
bih mogao nastaviti OCR/AI obradu bez pogrešnog parsera.

**Acceptance Criteria**

- Kada classifier ne može dovoljno sigurno odrediti tip dokumenta, tada sistem mora postaviti dokument u status
  `NEEDS_CLASSIFICATION_REVIEW`.
- Kada je dokument u statusu `NEEDS_CLASSIFICATION_REVIEW`, tada sistem ne smije dozvoliti pokretanje ekstrakcije dok
  tip nije ručno potvrđen.
...
- Nakon uspješne ručne potvrde tipa, sistem mora vratiti dokument u status iz kojeg se ekstrakcija može ponovo
  pokrenuti.
...
### US-8.17 - Validacija ekstraktovanih polja u zavisnosti od tipa dokumenta

**Opis** - Kao operater, želim da sistem validira ekstraktovana polja prema tipu dokumenta, kako pravila za fakture ne
bi pogrešno blokirala račune, bankovne izvode ili forme.
...
### US-8.18 - Prikaz proširenog document-processing toka na korisničkom interfejsu

**Opis** - Kao korisnik sistema, želim da upload, lista dokumenata i detalji dokumenta jasno prikazuju nove tipove
dokumenata, metapodatke za klasifikaciju i review status, kako bih razumio u kojem koraku obrade se dokument nalazi.
...
- Detail stranica mora prikazati classification metadata samo kada je classifier stvarno korišten ili kada dokument
  zahtijeva review.
