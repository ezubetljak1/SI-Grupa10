...
@Mapper(componentModel = "spring", imports = {DocumentType.class, DocumentStatus.class, LocalDateTime.class})
public interface DocumentMapper {

    @Mapping(target = "id", ignore = true)
    @Mapping(source = "createdByUserId", target = "createdBy")
    @Mapping(target = "uploadDate", expression = "java(LocalDateTime.now())")
    @Mapping(target = "documentType", expression = "java(DocumentType.valueOf(request.getDocumentType().toUpperCase()))")
    ...
    @Mapping(target = "name", expression = "java(request.getName() != null ? request.getName() : entity.getName())")
    @Mapping(
            target = "documentType",
            expression = "java(request.getDocumentType() != null ? DocumentType.valueOf(request.getDocumentType().toUpperCase()) : entity.getDocumentType())"
    )
    @Mapping(
            target = "documentStatus",
            expression = "java(request.getDocumentStatus() != null ? DocumentStatus.valueOf(request.getDocumentStatus().toUpperCase()) : entity.getDocumentStatus())"
    )
    DocumentEntity updateEntity(DocumentUpdateRequest request, @MappingTarget DocumentEntity entity);
